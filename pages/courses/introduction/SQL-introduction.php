<!DOCTYPE html>
<html lang="ru">
<head>
    <link rel="stylesheet" href="introduction.css">
    <link href="https://fonts.googleapis.com/css2?family=Roboto:ital,wght@0,100;0,300;0,400;0,500;0,700;0,900;1,100;1,300;1,400;1,500;1,700;1,900&display=swap" rel="stylesheet">
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>SQL-introduction</title>
</head>
<body>
<?php include_once($_SERVER['DOCUMENT_ROOT'] . "/blocks/header.php"); ?>
    <section class="container">
        <div class="cours__content">
            <div class="cours__left">
                <a class="lessons__list lessons__list-active" href="#">Основы SQL</a>
                <br><br><br>
                <a class="lessons__list" href="#">Работа с несколькими таблицами</a>
                <br><br><br>
                <a class="lessons__list" href="#">Продвинутый SQL</a>
            </div>
            <div class="cours__right">
                <div class="bakground__curs">
                </div>
            <div class="name__cours">
                <h1>SQL</h1>
                <a href="#">Courses &#8212 &gt SQL  &#8212 &gt Основы SQL</a>
            </div>
            <div class="leson__description">
                <h2>Основы SQL</h2>
                <p>Установка и настройка таблиц для упражнений. Создание таблиц. Связи между таблицами.Основные запросы SELECT.</p>
            </div>
            <div class="leson__video">
                <h2>Видео</h2>
                <video controls="controls">
                </video>
            </div>
        </div>
        </div>
    </section>
    <?php include_once($_SERVER['DOCUMENT_ROOT'] . "/blocks/footer.php"); ?>
</body>
</html>